...
//----------------------------------------------------------------------
__global__ void helloworld_thread(void){
	printf("Hello world from block %d, thread %d \n", blockIdx.x,threadIdx.x + blockIdx.x*blockDim.x);
}

__global__ void helloworld_3d(void){
	printf("Hello world from block (%d,%d,%d), thread %d \n", blockIdx.x, blockIdx.y, blockIdx.z, threadIdx.x +threadIdx.y + threadIdx.z + blockIdx.x*blockDim.x + blockIdx.y*blockDim.y + blockIdx.z*blockDim.z);
}
//----------------------------------------------------------------------
...
// write your kernel here
__global__ void helloworld_warp(void){
	//if(threadIdx.x%32 == 0){
