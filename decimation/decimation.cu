...
__global__ void decimation(int *d_input, int *d_GPU_sum2, int *d_GPU_sum4, int N){
	//Calculates GPU_sum
	int index = 2*(threadIdx.x+blockDim.x*blockIdx.x);
	d_GPU_sum2[index] = d_input[index] + d_input[index+1];
	d_GPU_sum4[index] = d_GPU_sum2[index] + d_GPU_sum2[index+1];
}
//----------------------------------------------------------------------
...
int main(void) {
	...
	cudaMalloc(&d_GPU_sum2, N*sizeof(*h_GPU_sum2));
	cudaMalloc(&d_GPU_sum4,N*sizeof(*h_GPU_sum4));
	cudaMalloc(&d_input,N*sizeof(*h_input));

	cudaMemcpy(d_input,h_input,N*sizeof(int),cudaMemcpyHostToDevice);	

	decimation<<<N/128,128>>>(d_input,d_GPU_sum2,d_GPU_sum4,N);
	
	cudaMemcpy(h_GPU_sum2,d_GPU_sum2,N*sizeof(int),cudaMemcpyDeviceToHost);
	cudaMemcpy(h_GPU_sum4,d_GPU_sum2,N*sizeof(int),cudaMemcpyDeviceToHost);
